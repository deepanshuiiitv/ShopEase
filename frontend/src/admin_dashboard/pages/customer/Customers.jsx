...
export default function Customers() {
  ...
  const [customers, setCustomers] = useState(() => {
    ...
    if (location.state && location.state.updatedCustomer) {
      return initialCustomers.map((c) =>
        c.id === location.state.updatedCustomer.id
  ...
  const confirmEdit = () => {
    navigate('/admindashboard/edit-customer', {
      state: { customer: modal.payload },
    });
    setModal({ open: false, action: null, payload: null });
  };
  ...
  const confirmAdd = () => {
    navigate('/admindashboard/add-customer');
    setModal({ open: false, action: null, payload: null });
  };

  const handleView = (customer) => {
    navigate('/admindashboard/customer-detail', { state: { customer } });
  };
        ...
        <table className="min-w-full divide-y divide-blue-200">
          ...
          <tbody className="bg-white divide-y divide-blue-100">
            {customers.map((customer, idx) => (
              <tr
                key={customer.id}
                className={idx % 2 === 0 ? 'bg-blue-50 hover:bg-blue-100' : 'hover:bg-blue-100'}
              >
                <td className="px-6 py-4 text-sm font-medium text-blue-900">{customer.id}</td>
                <td className="px-6 py-4 text-sm text-blue-900">{customer.name}</td>
                <td className="px-6 py-4 text-sm text-blue-900">{customer.email}</td>
                <td className="px-6 py-4 text-sm text-blue-900">{customer.joined}</td>
                <td className="px-6 py-4 text-sm text-blue-900 flex gap-2">
                  <button
