...
export default function Admins() {
  ...
  const [admins, setAdmins] = useState(() => {
    ...
    if (location.state && location.state.updatedAdmin) {
      return initialAdmins.map((a) =>
        a.id === location.state.updatedAdmin.id
          ? { ...location.state.updatedAdmin }
          : a
      );
    }
  ...
  const confirmEdit = () => {
    navigate('/admindashboard/edit-admin', { state: { admin: modal.payload } });
    setModal({ open: false, action: null, payload: null });
  };
  ...
  const confirmAdd = () => {
    navigate('/admindashboard/add-admin');
    setModal({ open: false, action: null, payload: null });
  };

  const handleView = (admin) => {
    navigate('/admindashboard/admin-detail', { state: { admin } });
  };
        ...
        <table className="min-w-full divide-y divide-blue-200">
          ...
          <tbody className="bg-white divide-y divide-blue-100">
            {admins.map((admin, idx) => (
              ...
              >
                <td className="px-6 py-4 text-sm font-medium text-blue-900">{admin.id}</td>
                <td className="px-6 py-4 text-sm text-blue-900">{admin.name}</td>
                <td className="px-6 py-4 text-sm text-blue-900">{admin.role}</td>
                <td className="px-6 py-4 text-sm text-blue-900">{admin.status}</td>
